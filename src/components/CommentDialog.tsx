...
export default function CommentDialog({ isOpen, onClose, onPost, parentTweetId }: CommentDialogProps) {
  ...
  const handlePost = async () => {
    if (!isValid) return;
    if (!isAuthenticated || !user?.id) return showError("Authentication error");

    setIsPosting(true);

    try {
      await new Promise((resolve) => setTimeout(resolve, 300));
      console.log("Comment draft saved", { parentTweetId, text: content.trim() });
      onPost?.(content);
      setContent("");
      onClose();
      showSuccess("Comment saved locally.");
    } catch (error) {
